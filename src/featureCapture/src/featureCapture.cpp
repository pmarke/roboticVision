...
void FeatureCapture::findFeaturesGFTT(){
	...
	cv::goodFeaturesToTrack(_grayImg,_corners,_gftt.maxCorners,_gftt.qualityLevel,_gftt.minDistance,cv::Mat(),_gftt.blockSize,_gftt.useHarrisDetector,_gftt.k);
	
	for(int i = 0; i < _corners.size(); i++){ // draw in the circles
		cv::circle(_imgCopy, _corners[i], 5, cv::Scalar(204,102,0),2,2,0);
	}
